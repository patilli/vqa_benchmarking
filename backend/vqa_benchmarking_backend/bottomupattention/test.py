...
from bottomupattention.utils.utils import mkdir, save_features
from bottomupattention.utils.extract_utils import get_image_blob, save_bbox, save_roi_features_by_bbox, save_roi_features
from bottomupattention.utils.progress_bar import ProgressBar
from bottomupattention.models import add_config
from bottomupattention.models.bua.box_regression import BUABoxes
...
def parse_roi_features(cfg, im_file, im, dataset_dict, boxes, scores, features_pooled, attr_scores=None):
...
@ray.remote(num_gpus=0.25, num_cpus=3)
def extract_feat_in_memory(model, im_file, cfg, args):
    model.eval()
    im = cv2.imread(os.path.join(args.image_dir, im_file))
    if im is None:
        raise Exception(f'{os.path.join(args.image_dir, im_file)} is illegal!')
    dataset_dict = get_image_blob(im, cfg.MODEL.PIXEL_MEAN)
    # extract roi features
    attr_scores = None
    with torch.set_grad_enabled(False):
    ...
    if not attr_scores is None:
        attr_scores = [attr_score.cpu() for attr_score in attr_scores]
    return parse_roi_features(cfg, im_file, im, dataset_dict, 
        boxes, scores, features_pooled, attr_scores)
...
def main():
    ...
    parser.add_argument(
        "--config-file",
        default="bottomupattention/configs/bua-caffe/extract-bua-caffe-r101.yaml",
        metavar="FILE",
        help="path to config file",
    )

    parser.add_argument('--num-cpus', default=12, type=int, 
    ...
    parser.add_argument('--image-dir', dest='image_dir',
                        ...
                        default="demo")
    parser.add_argument('--bbox-dir', dest='output_dir',
